...
let createRequestUri = () => {
  var uri = window.location.protocol + "//" + window.location.host + "/actions/time";
...
let displayRedcords = (records) => {
  ...
  records.forEach(record => {
    if( record.seconds > max ){
      max = record.seconds;
    }
  });
  // 表示最大値を100の位で切り上げた値に設定
  var maxSeconds = Math.ceil(max / 100) * 100;
  ...
  records.forEach(record => {
    ...
    var widthRate = record.seconds / maxSeconds * 100;
    if (widthRate >= 75 ){
    ...
    span.textContent = record.seconds;
...
// イベントに対応する処理の追加
window.addEventListener('load', initTimes);